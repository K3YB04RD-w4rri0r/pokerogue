...
export class MockSprite implements MockGameObject {
  ...
  destroy() {
    ...
    // (Detaching from our parent container's display list is handled by the
    // destroy wrapper installed in MockContainer.trackChild() when we were
    // add()ed — mirroring real Phaser's parentContainer.remove(this) on destroy.)
